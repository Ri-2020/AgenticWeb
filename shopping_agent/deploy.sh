...
# ============================================================
# Deploy crewAI agent
# - cloud: Cloud Run service with Pub/Sub push trigger
# - local:  local Pub/Sub pull listener only
# ============================================================
# Usage:
#   ./deploy.sh
#   ./deploy.sh local
...
SERVICE_NAME="shopassist-agent"
PUBSUB_TOPIC="${PUBSUB_TOPIC:-shopping-agent-jobs}"
PUSH_SUBSCRIPTION_NAME="${PUSH_SUBSCRIPTION_NAME:-shopping-agent-jobs-push}"
LOCAL_SUBSCRIPTION_NAME="${LOCAL_SUBSCRIPTION_NAME:-shopping-agent-jobs-local}"
...
# Read env values (.env overrides shell)
...
if [ "$MODE" = "cloud" ]; then
  ...
  echo "    Pub/Sub push subscription: $PUSH_SUBSCRIPTION_NAME -> $AGENT_URL"
  exit 0
fi
...
if [ "$MODE" = "local" ]; then
  ...
  echo "==> Ensuring local Pub/Sub pull subscription '$LOCAL_SUBSCRIPTION_NAME' exists..."
  if gcloud pubsub subscriptions describe "$LOCAL_SUBSCRIPTION_NAME" >/dev/null 2>&1; then
    echo "    Subscription already exists."
  else
    gcloud pubsub subscriptions create "$LOCAL_SUBSCRIPTION_NAME" \
      --topic "$PUBSUB_TOPIC"
  fi

  echo ""
  echo "==> Starting local Pub/Sub pull listener (no HTTP server)..."
